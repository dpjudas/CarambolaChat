...
bool XMLTokenizer_Impl::next_tag_node(XMLToken *out_token)
{
	...
	else // out_token->type == XMLToken::ELEMENT_TOKEN
	{
		...
		while (true)
		{
			...
			if (pos == data.npos || pos == size - 1)
				XMLTokenizer_Impl::throw_exception("Premature end of XML data!");
			if (data[pos++] != '=')
				XMLTokenizer_Impl::throw_exception(uicore::string_format("XML error(s), parser confused at line %1 (tag=%2, attributeName=%3)", get_line_number(), out_token->name, attributeName));
	...
	if (data[pos] != '>')
		XMLTokenizer_Impl::throw_exception(uicore::string_format("Error in XML stream, line %1 (expected end of tag)", get_line_number()));
	pos++;
...
bool XMLTokenizer_Impl::next_exclamation_mark_node(XMLToken *out_token)
{
	...
	if (data.compare(pos, 7, "DOCTYPE") == 0)
	{
		...
		if (data[pos] != '[' && data[pos] != '>')
		{
			...
			else if (data.compare(pos, 6, "PUBLIC") == 0)
			{
				...
				XMLTokenizer_Impl::throw_exception(uicore::string_format("Error in XML stream, line %1 (unknown external identifier type in DOCTYPE)", get_line_number()));
		...
		if (data[pos] == '[')
		{
			...
			if (subset_end == data.npos)
				XMLTokenizer_Impl::throw_exception(uicore::string_format("Error in XML stream, line %1 (expected end of internal subset in DOCTYPE)", get_line_number()));
		...
		if (data[pos] != '>')
			XMLTokenizer_Impl::throw_exception(uicore::string_format("Error in XML stream, line %1 (expected end of DOCTYPE)", get_line_number()));
		pos++;
	...
	{
		XMLTokenizer_Impl::throw_exception(uicore::string_format("Error in XML stream at position %1", static_cast<int>(pos)));
		return false;
	}
}
